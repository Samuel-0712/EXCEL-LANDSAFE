...
### 4.5 About
**Decision: About becomes a dedicated page (`/about`), not a same-page homepage section.** Requires React Router (`react-router-dom`) if not already in the stack — "About" in the nav links to `/about` instead of an anchor scroll. Since deploying on Vercel, confirm the routing rewrite/fallback is configured so refreshing `/about` directly doesn't 404 (flagged back in the deploy checklist).

**4.5a — Homepage teaser (stays on the single-scroll homepage):**
- Short version only: photo one side, 2-3 sentence intro copy the other (who they are, why "seamless")
- Optional understated 3-stat row (e.g. "500+ Trips Planned," "10+ Years," "24/7 Support") — no gimmicky animated counters
- A "Learn Our Story →" link/CTA leading to the full `/about` page — this is the teaser's only job, not to carry the full mission/vision/leadership content
- Soft secondary CTA or none otherwise — About doesn't need aggressive selling

**4.5b — Full `/about` page structure:**
1. **Page hero** (smaller than homepage hero) — headline could use the real tagline "Your Journey, Our Commitment!", a complementary but distinct photo from the homepage hero
2. **Company story** — the real "About Us" paragraph from the company profile, given full room to breathe rather than condensed for a homepage card:
...
3. **Mission & Vision — full statements**, not the trimmed card versions used elsewhere. Two-card or stacked layout with real visual weight:
...
4. **Core Values** — clean icon-list grid (reusing existing icon language/style, same green/charcoal palette): Commitment to Excellence, Customer-Centric Approach, Continuous Learning and Growth, Integrity and Transparency, Team Spirit
5. **Leadership team** — text-only treatment for now, no photos: two-card layout, name + title only, no image placeholder/avatar. Keep the card simple — name in bold, title beneath in the muted body style used elsewhere (e.g. the testimonial attribution line):
   - Oluwayomi Elizabeth Dada — Managing Director
   - Babatunde Leo Dada — Managing Partner
   - Headshots can be added later if/when the client provides them — design the cards so a photo could slot in cleanly without restructuring, but don't block on it now
6. **Facts strip** — quiet, text-led, consistent with the accreditation line treatment:
   - Founded 2011
   - Address: 60 Allen Avenue, by Adeboye Sholanke Entrance Gate, Ikeja, Lagos
   - CAC Registered · NANTA Member
7. **Closing CTA band** — same pattern as the homepage's final CTA band, so the page doesn't dead-end without a path back to inquiry

**Note:** this changes the earlier plan of folding Mission/Vision + Accreditation into a compact homepage card row — that content now lives in full on `/about`, with only a lightweight teaser + link remaining on the homepage itself.
...
- ~~Exact service list~~ — **Resolved:** confirmed via official company profile (Flight Bookings, Hotel Reservations, Tour Packages, Airport Logistics, Corporate Travel Management, Visa Consultation, Travel Insurance Solutions, Cruise Bookings, Customized Travel Experiences) and separately via a 9-item scope list — see Section 12's "Full 9-service scope reconciliation"
- Real copy for remaining sections (Services card descriptions, some destination detail) — About/Mission/Vision now resolved (see Section 4.5)
- Hero image/video — client-provided or curated stock
- ~~Whether real testimonials exist to include~~ — **Resolved:** 6 real testimonials sourced from client's Instagram, see Section 4.6
- ~~Years in operation~~ — **Resolved:** founded 2011
- ~~Physical office address~~ — **Resolved:** 60 Allen Avenue, by Adeboye Sholanke Entrance Gate, Ikeja, Lagos
- Whether the "Website: Processing" line in the client's company profile means they're expecting a different site/domain arrangement than this build — worth a quick clarifying check, not assumed
